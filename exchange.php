...
abstract class Exchange
{
    ...
    * To use the getExchangeSymbol() with keys as standard symbols 
    * It is assigned in the setSymbols() function and retrieved with the getSymbols() function.
    * To use the getExchangeSymbol($symbol) the keys of the array must be set with standard format of only capital letters
    * i.e. array('DOTBTC':"DOT/BTC", 'LTCBTC':"LTC/BTC", 'DOGEBTC':"DOGE/BTC",  ...) 
    * note: array can have different values/formats for different exchanges: "LTC/BTC" "LTCBTC" "ltc_btc"
    *       but keys should be the same for equivalent currency pairs
    *
   ...
   // @todo not implemented for all exchanges, setSymbols() must create associative array $this->symbols with keys as standard symbols and values as exchange symbols or symbolids.
