...
class Cryptopia extends Exchange{
   ...
   public function __construct($priv, $pub) {
      ...
      if ($priv) { // do not check connection for public API only with no Keys
   ...
   public function updatePrices() {
      ...
      foreach($json as $pair) {
         $standardSymbol = $this->makeStandardSymbol( $pair["Label"] );
         $this->prices[$standardSymbol]['high'] = $pair['High'];
         $this->prices[$standardSymbol]['low']  = $pair['Low'];
         $this->prices[$standardSymbol]['bid']  = $pair['BidPrice'];
         $this->prices[$standardSymbol]['ask']  = $pair['AskPrice'];
         $this->prices[$standardSymbol]['last'] = $pair['LastPrice'];
         $this->prices[$standardSymbol]['time'] = '';  // not available on Cryptopia
      }
   }
   ...
   public function tradeHistory( $symbol, $type = '' )
   {
      $apiParams = array( 'TradePairId'=>$this->getExchangeSymbol($symbol) );       
      $myOrders = json_decode($this->apiCall("GetTradeHistory", $apiParams), true);  
      //print_r(array_slice($myOrders['Data'], 0, 5));

      $orders = array();
      if( $myOrders['Success'] == "true" && $myOrders['Error'] == "") {
         foreach ($myOrders['Data'] as $order) {
            if($type == '' || $order["Type"] == $type) { // if type is set only get orders of that type
               $orderSymbol = $this->makeStandardSymbol($order["Market"]); // convert to standard format currency pair
               $orders[] = ["symbol"=>$orderSymbol, "type"=>$order["Type"], "price"=>$order["Rate"],
                            "amount"=>$order["Amount"], "time"=>strtotime($order["TimeStamp"]) ];
            }
         }
      } else {
         throw new Exception("Can't get active orders, Error: " . $myOrders['Error'] );
   ...
   public function cancelOrder($id) {
      ...
      $result = json_decode($result, true);
      if( $result['Success'] == "true" ) {
         echo "Order Canceled: " . implode( ", ", $result['Data']) . "\n";
      } else {
